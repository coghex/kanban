...
- Claude origin: the Codex reviewer.
- Codex origin: the Claude reviewer.
- Unknown origin: both canonical reviewers independently.

The coordinator does not pin or verify which specific model backs either reviewer; it selects only the brand (which of `codex`/`claude` runs), deferring to that installation's own configured default. The published comment/marker records the reviewer key as verified fact and does not claim a specific model.
...
Return the PR number, origin or unknown status, issue-gate evidence, reviewer route, verdict, reviewed head, per-reviewer blockers, comment URL/status, and label state.
