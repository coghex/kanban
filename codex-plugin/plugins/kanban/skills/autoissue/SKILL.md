...
3. Preserve `$issue-review` stop conditions:
   ...
   - On `CHANGES_REQUESTED`, report the result and direct the issue through `$issue-rereview <issue>`; do not rerun the unchanged issue. That repair-and-rereview workflow is this bundle's own packaged skill; Kanban's `docs/drafting-workflow-contract.md` declares it and keeps the readiness gate separate from repair.
