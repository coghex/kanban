...
- Confirm every `✓` is backed either by a reconciliation entry whose `approved` is true or, only for a top-level `busy` result, by the issue carrying the exact `approval_label` and not the exact `changes_requested_label` reported by that document in the verified-complete snapshot.
