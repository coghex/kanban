...
argument-hint: "[optional: design document path, subject, or design notes]"
---
...
Never overwrite or repurpose an unrelated document. If `$ARGUMENTS` supplies
neither an idea nor an existing path, ask only what they want to design.

## Work from the user's thinking

Treat everything in `$ARGUMENTS` as source material, even when it is a
...
Use the `Edit` tool for every document edit. Preserve unrelated user edits and
