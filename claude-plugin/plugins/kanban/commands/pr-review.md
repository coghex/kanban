...
Require one positive PR number in `$ARGUMENTS`. Kanban already spawned this session as the canonical opposite-brand reviewer — its own CLI invocation pinned this session's model before this workflow ever ran — so for the normal known-origin case you perform the review yourself; the bundled coordinator only handles safe publication. Do not independently comment, label, or compensate for the coordinator's result.
