...
#430, #462, #511, #544, #575, and #576 the `/triage` roadmap workflow, its
`/retriage` refresh,
...
control surface, the `/fix` approved-pull-request workflow, the
`/finalize` manual merge fallback, the `/janitor` pipeline
housekeeping audit, the `/autosolve` autonomous solve-and-review loop, and —
since issue #685 — the `/auto-project-review` serial history audit that repeats
`/project-review` a counted or open-ended number of times,
each rendered
into both bundles from one authored source by
`tools/render_command_sources.py`. `/autosolve` is the last of the eight
commands that arc vendors, so with it every personal copy of a Kanban workflow
command is retired. It exists so a
...
`kanban@kanban` should be listed, and all twenty-six workflow names should be
...
`/project-review`, `/drain-prs`, `/fix`, `/finalize`, `/janitor`,
`/autosolve`, and `/auto-project-review`.
...
#127). The other twenty-one are drafting, readiness-gate, document, roadmap,
...
[docs/document-workflow-contract.md](../docs/document-workflow-contract.md).
`/repair` is not part of either declared surface. Only those twenty-one are
...
| `commands/project-review.md` | `/project-review` | Reviews **exactly one merged pull request per successful invocation**, judging it against the issue it claimed to satisfy, its commits, and a detached temporary worktree pinned to the fetched remote default-branch head — never the primary checkout's HEAD. The pull request is chosen and claimed by `scripts/project_review_ledger.py` from a complete paginated merged-PR inventory, under a lease whose liveness is `scripts/project_review_liveness.py`'s keeper; the completed attempt, its verification commit and any report are recorded back into `docs/project_review/ledger.md` as one checkpoint the helper makes. **Report-only**: it never creates or edits a tracker issue, and a review with at least one new or recurring finding writes one ledger-allocated `docs/project_review/<PR>.md` report in the branch-resolved `docs-wip` worktree for later `/process-report` disposition. Never repeats: there is no `continue`, and nothing transitions automatically into the direct-commit mode, which reviews pre-PR first-parent history on its own explicit request against the same ledger's `direct` key, under the same lock and with no claim. Resolves the repository once and names it on every `gh` call. Paired with the Codex `$project-review` skill. |
...
login `coghex`; every other comment comes back as metadata alone, so an
untrusted body never enters the solving session's context. The logins `claude`
and `codex` are unaffiliated accounts, not this pipeline's agents, and like
repository role, `author_association`, issue authorship, display name, and bot
status they grant nothing, and the set is hardcoded, so widening it costs a
reviewed pull request. `/solve` resolves the helper at
...
setup is the plugin being enabled with `disableAllHooks`
...
the same canonical `gpt-6-sol`/`claude-opus-5-5` at `xhigh` values the
...
- the marketplace and plugin manifests are valid and point at this
  directory;
- the commands directory contains exactly the twenty-six packaged workflows,
...
surface (all twenty-six commands under
`claude-plugin/plugins/kanban/commands/`)
and
all nine bundled Python assets — the review coordinator and its
model-roster reader, `/solve`'s trusted-comment helper, the three
document-workflow modules, `/project-review`'s ledger helper and
