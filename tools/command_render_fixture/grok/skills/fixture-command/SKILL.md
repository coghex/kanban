...
1. Take the issue number from `$ARGUMENTS`, which Grok substitutes with the
   text after /fixture-command before this skill runs.
2. Resolve this bundle's vendored helper through `$GROK_PLUGIN_ROOT` when Grok
   set it, otherwise under `$GROK_HOME/installed-plugins/kanban-<hash>/`
   (default `~/.grok`).

This third rendering is issue #716's proof that a brand may combine the
skill-directory layout with the `/` sigil and a frontmatter key set of its own.
It lands under `tools/` like the other two, so no bundle gains a workflow.
