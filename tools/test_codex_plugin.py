...
class ReviewerSourceIsolationTests(unittest.TestCase):
    ...
    def test_extract_source_result_is_read_only(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            destination.mkdir()
            module.extract_source(REPO_ROOT, "coghex/kanban", 0, head, destination)
    ...
    def test_self_review_known_origin_returns_context_without_spawning(self):
        ...
        with mock.patch.object(module, "resolve_repository", return_value="coghex/kanban"), mock.patch.object(
            ...
            code, result = module.workflow(
                ...
                allow_no_issue=False,
                self_review=True,
                # A claude-origin PR routes to codex, so a codex caller is the
                # one session entitled to self-review it (issue #303).
        ...
        for name in ("pr-rereview", "pr-revise", "repair"):
            ...
            for action in expected_actions:
                ...
                self.assertEqual(
                    len(matching_blocks),
                    1,
                    f"{name}/SKILL.md must have exactly one {action} coordinator invocation",
                )
                self.assertIn(
                    "--allow-no-issue",
                    matching_blocks[0],
                    f"{name}/SKILL.md {action} invocation must preserve the standalone PR gate mode",
                )
...
class SolveStopConditionScopeTests(unittest.TestCase):
    ...
    def test_the_review_prohibition_stays_absolute(self):
        text = (SKILLS_ROOT / "solve" / "SKILL.md").read_text(encoding="utf-8")
