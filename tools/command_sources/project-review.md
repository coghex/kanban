...
findings report. That report is the durable handoff to {{cmd:process-report}},
...
<!-- brand:codex -->
```bash
CURSOR="$(find "${CODEX_HOME:-$HOME/.codex}/plugins/cache" -path '*/kanban/*/skills/project-review/scripts/project_review_cursor.py' 2>/dev/null | head -n1)"
[ -n "$CURSOR" ]
```
<!-- /brand -->
