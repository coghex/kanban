...
Repetition is {{cmd:auto-project-review}}'s, not this workflow's: there is no
...
{{cmd:janitor}} take it once they have stopped. Deleting a worktree out from
...
3. Search open and closed tracker issues for context and deduplication — once
   ...
   **An already-tracked finding is still a finding.** The tracker search
   ...
   {{cmd:process-report}} filing a second issue for it — the report is the
...
4. Read the reports the row's own history links and compare the finding with
   ...
   - **Repeated** — an earlier report's `PRR-k` describes it and it is still
     ...
     defect across two entries that {{cmd:process-report}} would then dispose of
     twice.
...
batch. A user who wants another review invokes {{cmd:project-review}} again;
{{cmd:auto-project-review}} is what repeats it without being asked each time.
