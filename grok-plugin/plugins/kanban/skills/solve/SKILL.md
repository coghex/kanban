---
name: solve
description: Claim a GitHub issue, implement its minimal verified fix in an isolated worktree, and open a grok-origin PR. Use when the user invokes /solve or explicitly asks to take an issue through PR creation. This is the Grok brand; never follow a Claude or Codex solve skill that would stamp a different origin marker.
...
3. Fetch the effective spec through this bundle's vendored trusted-comment helper before editing. It returns the COMPLETE paginated comment timeline in chronological order while keeping untrusted comment bodies out of this session. Kanban and Grok invoke this workflow with the *worked* repository as the working directory, not this plugin's own install location, so locate the installed helper by searching under `$GROK_HOME` (default `~/.grok`) rather than a path relative to the current directory:

   ```bash
   TRUSTED_SPEC="$(find "${GROK_HOME:-$HOME/.grok}" -path '*/kanban/skills/solve/scripts/trusted_issue_spec.py' 2>/dev/null | head -n1)"
