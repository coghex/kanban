...
- [x] OVF-3. Add the `f` fullscreen toggle for open overlays — [#543]
...
- **Acceptance signals:** golden fullscreen frames for a details and a
  ...
  existing overlay frames change only by the `f` chip their footer row
  gains; incidents elision holds at fullscreen width;
