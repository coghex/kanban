...
`plugins/cache` tree inside it. Every Codex skill that resolves a bundled script
that way is a consumer and appears in the row, `$project-review`'s sweep cursor
included. It is `external`/`mandatory: no` for that
