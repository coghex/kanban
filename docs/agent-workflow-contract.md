...
- **Cross-brand handoff model policy:** for known-origin `$pr-review`/
  ...
  nested reviewer to the exact `gpt-6-sol`/`claude-opus-5-5` at `xhigh`
...
- **Review authority and the verdict a merge obeys:** the drainer reads every
  ...
  a changed contract: a later canonical `pr-review:v2` approval of that head
  whose first-line owner-directive record names a directive the rejection was
  not reviewed under lifts it, because the owner replaced the requirements it
  judged (§ "Owner directive"). A rejection naming any other head imposes
...
- **Project-scoped assets are preferred.** Where Kanban must write outside
  ...
  *is*, with the expected relative name selecting each managed slot and its
  ...
  neither moves nor closes, and a drainer currently holding that lock
  classifies the state
  `external`. Holding it is what reaches that branch, not the PID its document
...
- fails if either bundle's copy of the project-review ledger helper
