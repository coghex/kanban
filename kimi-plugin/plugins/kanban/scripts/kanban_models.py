...
Claude, Codex, Grok, Kimi, and Google plugins' review_pr.py.
...
This module ships in six homes, held byte-identical the way `kanban_config.py`'s
...
`kimi-plugin/plugins/kanban/scripts/kanban_models.py`, and
...
and must load the reader from beside itself. The Claude, Grok, Kimi, and Google
coordinator copies resolve `roles.pr_review` cells and pin them, while the Codex
one reads only `agents` for the operating mode and still passes no model or
effort to anything (D-2).
"""
