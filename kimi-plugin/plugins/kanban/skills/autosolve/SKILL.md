...
at approval; never merge or finalize. This session is Kimi. It must never
...
- **Needs a documentation landing workflow this bundle does not ship** —
  ...
  not reclaim the issue; /solve already released it. This Kimi bundle
...
The body's final non-whitespace content must be `<!-- pr-origin:kimi -->`,
...
pull request /solve opened from a different push remote is still kimi-origin:
...
The pull request's kimi origin marker requires a fresh **Codex** review. Do
...
Kimi, and it authored the kimi-origin pull request it is now trying to get
...
Prefer `$KIMI_PLUGIN_ROOT` when the launcher set it — that is the plugin
...
Claude, Codex, or Grok plugin path, a checkout-relative path, or a personal
copy.
...
The dry run must report `"origin": "kimi"` and `"route": "codex"`. If it
...
Then run the real round with the same flags plus `--expected-origin kimi` and
...
python3 "$COORDINATOR" \
  ...
  --repo "$REPO" \
  --review "$PR" \
  --expected-origin kimi \
  --expected-route codex \
  --json
```

A `"status": "route_mismatch"` result means the live origin or route is no
longer kimi/codex; stop and report it. Do not retry without those flags, and
...
- `reviewed:changes`: read the current output or comment, fix every blocking
  ...
  required check. Kimi revises in this same session; do not ask the board to
...
This session's own brand is `kimi`, so a marker reading `reviewers=kimi` on
this kimi-origin pull request is the publication failure that last
