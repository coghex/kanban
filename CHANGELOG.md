...
- The tracked Grok plugin (`grok-plugin/`) packages `/solve` and `/autosolve`.
  ...
  not a Kanban-spawned provider. Helper lookup uses `$GROK_PLUGIN_ROOT` or
  the hashed `$GROK_HOME/installed-plugins/kanban-<hash>/` layout.
- `<!-- pr-origin:grok -->` is a known pull-request origin. Dual-mode review
  ...
  `$finalize` / `/finalize` accept a Codex-only approval on that origin and
  still require a dual-brand marker when no origin is declared.
