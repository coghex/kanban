...
import Control.Exception (Exception, IOException, SomeException, bracket, mask, onException, throwIO, try, uninterruptibleMask_)
...
import System.IO (BufferMode (LineBuffering), Handle, IOMode (ReadMode, WriteMode), hClose, hSetBinaryMode, hSetBuffering, openFile)
...
launchPullRequestWorker repository number origin action existingSession existingLogPath provenance userMessage parent configPath workflowConfig = do
  now <- getCurrentTime
