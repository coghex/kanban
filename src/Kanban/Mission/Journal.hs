...
data MissionJournalLine
  ...
  | -- | A complete line carrying a @schemaVersion@ this release does not
    ...
    --
    -- No /reader/ ever sees one. @readMissionJournal@ drops these before it
    ...
    -- nothing to decide. The one caller that does see them decodes lines
    -- itself and is not reading them:
    -- @Kanban.Mission.Store.unprovenRecordRefusals@ is about to /remove/ the
    -- file, and silence is not permission to destroy a record a later release
    -- — or, under the ambiguous pre-#615 root, another repository — wrote. The
    -- case exists here because this is where the decision is made, and because
    -- it is what separates a line another release wrote from a line that is
    -- broken.
