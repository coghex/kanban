...
discoverDrainerController repository = do
  ...
  case resolved of
    ...
    Right (DrainerLaunchd, plist) -> do
      result <- runGroupedProcess drainerControllerSubject (Just discoveryTimeoutSeconds) "/usr/bin/plutil" ["-extract", "ProgramArguments", "json", "-o", "-", plist]
      pure $ do
...
runDrainerCommand seconds controller command = do
  result <-
    runGroupedProcess
      drainerControllerSubject
      (Just seconds)
      controller.controllerExecutable
      (controller.controllerArguments <> ["--json", command])
  pure $ case result of
    Left failure -> Left (invocationFailureMessage seconds ("drainer " <> Text.pack command) (serviceTransitionCommand command) failure)
...
-- they are about. Stated once here rather than passed at each call site, so a
-- drainer message is never the shape "the issue approval controller led".
drainerControllerSubject :: Text
drainerControllerSubject = "the drainer controller"
...
runDirectMerge scriptPath repository configPath number = do
  ...
  case python of
    ...
    Just pythonPath -> do
      result <- runGroupedProcess drainerControllerSubject Nothing pythonPath (directMergeArguments scriptPath repository configPath number)
      pure $ case result of
